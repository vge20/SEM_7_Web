...
public class UpdateAccountPasswordScanState extends State {
    private Logger logger;

    public UpdateAccountPasswordScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (update.hasMessage() && update.getMessage().hasText() && update.getMessage().getText().equals("/start")) {
            ...
            userTmp.setPrivilegeLevel(user.getPrivilegeLevel());

            boolean flag = false;
            try {
                flag = userService.updateUser(userTmp);
            } catch (Exception e) {
                logger.error("Ошибка " + e + " при обновлении пользователя!");
            }
            if (flag)
                userView.updateUserSuccessMessage(update);
            else
                userView.updateUserErrorMessage(update);
