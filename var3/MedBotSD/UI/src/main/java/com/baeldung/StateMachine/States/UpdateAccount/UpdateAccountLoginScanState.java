...
public class UpdateAccountLoginScanState extends State {
    private Logger logger;

    public UpdateAccountLoginScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (update.hasMessage() && update.getMessage().hasText() && update.getMessage().getText().equals("/start")) {
            ...
            userTmp.setLogin(update.getMessage().getText());
            userView.userPasswordInputMessage(update);
            return enumStates.UPDATE_ACCOUNT_PASSWORD_SCAN_STATE;
        }
