...
public class UpdateDoctorLastNameScanState extends State {
    private Logger logger;

    public UpdateDoctorLastNameScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (update.hasMessage() && update.getMessage().hasText() && update.getMessage().getText().equals("/start")) {
            ...
            doctorTmp.setLastName(update.getMessage().getText());
            doctorView.doctorGenderInputMessage(update);
            return enumStates.UPDATE_DOCTOR_GENDER_SCAN_STATE;
        }
