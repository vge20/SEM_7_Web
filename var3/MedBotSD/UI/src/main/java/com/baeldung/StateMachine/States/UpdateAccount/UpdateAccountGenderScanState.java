...
public class UpdateAccountGenderScanState extends State {
    private Logger logger;

    public UpdateAccountGenderScanState(Logger logger) {this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (update.hasMessage() && update.getMessage().hasText() && update.getMessage().getText().equals("/start")) {
            ...
            if (gender.equals("ж")) {
                userTmp.setGender(false);
                userView.userBirthDateInputMessage(update);
                return enumStates.UPDATE_ACCOUNT_BIRTH_DATE_SCAN_STATE;
            }
