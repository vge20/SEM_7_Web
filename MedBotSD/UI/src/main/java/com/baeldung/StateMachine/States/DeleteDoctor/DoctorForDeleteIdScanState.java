package com.baeldung.StateMachine.States.DeleteDoctor;
...
public class DoctorForDeleteIdScanState extends State {
    private Logger logger;

    public DoctorForDeleteIdScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (update.hasMessage() && update.getMessage().hasText() && update.getMessage().getText().equals("/start")) {
            ...
            if (id < 0) {
                doctorView.deleteDoctorErrorMessage(update);
                ...
                flag = false;
                try {
                    flag = doctorService.deleteDoctor(id);
                } catch (Exception e) {
                    logger.error("Ошибка " + e + " при удалении врача!");
                    doctorView.deleteDoctorErrorMessage(update);
                    flag = false;
                }

                if (flag)
                    doctorView.deleteDoctorSuccessMessage(update);
