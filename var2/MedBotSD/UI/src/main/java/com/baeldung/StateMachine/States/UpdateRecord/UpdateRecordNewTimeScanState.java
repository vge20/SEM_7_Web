...
public class UpdateRecordNewTimeScanState extends State {
    private Logger logger;

    public UpdateRecordNewTimeScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (setTimeFlag) {
            boolean flag = false;
            try {
                flag = recordService.updateRecord(recordTmp);
            } catch (Exception e) {
                logger.error("Ошибка " + e + " при изменении записи!");
            }

            if (flag)
                recordView.updateRecordSuccessMessage(update);
            else
                recordView.updateRecordErrorMessage(update);
