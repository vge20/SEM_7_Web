...
public class UpdateAccountFirstNameScanState extends State {
    private Logger logger;

    public UpdateAccountFirstNameScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (update.hasMessage() && update.getMessage().hasText() && update.getMessage().getText().equals("/start")) {
            ...
            return enumStates.UPDATE_ACCOUNT_LAST_NAME_SCAN_STATE;
        }
