...
public class DeleteRecordTimeScanState extends State {
    private Logger logger;

    public DeleteRecordTimeScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (setTimeFlag) {
            ...
            try {
                recordsList = recordService.getRecordsByDate(recordTmp.getIdDoctor(), recordTmp.getDate());
                for (int i = 0; i < recordsList.size(); i++)
                    if (recordsList.get(i).getIdUser() == recordTmp.getIdUser() &&
                            recordsList.get(i).getStartTime().equals(recordTmp.getStartTime()) &&
                            recordsList.get(i).getEndTime().equals(recordTmp.getEndTime())) {
                        recordTmp.setId(recordsList.get(i).getId());
                    }

                flag = recordService.deleteRecord(recordTmp);
            } catch (Exception e) {
                logger.error("Ошибка " + e + " при удалении записи!");
            }

            if (flag)
                recordView.deleteRecordSuccessMessage(update);
            else
                recordView.deleteRecordErrorMessage(update);
