...
public class RegistrationBirthDateScanState extends State {
    private Logger logger;

    public RegistrationBirthDateScanState(Logger logger) { this.logger = logger; }
    ...
    public int updateHandler(int state, EnumStates enumStates, Update update, User user, User userTmp, Doctor doctorTmp,
        ...
        if (update.hasMessage() && update.getMessage().hasText() && update.getMessage().getText().equals("/start")) {
            ...
            if (birthDate != null) {
                ...
                return enumStates.REGISTRATION_LOGIN_SCAN_STATE;
            }
            else {
                userView.startUserMenu(update);
                return enumStates.START_MENU_STATE;
